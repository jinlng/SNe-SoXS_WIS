...
distance_modulus = 36.91       # checked
distance_pc = distances_conversions.DM_to_pc(distance_modulus)   # sn distance in pc
EBV = 0.036282      # checked
explosion_date = 2458336.3422034574
data_dicts = Read_data.read_data_Marshall_simple(
    '/home/jinlng/test_dir/Type_IIn/ZTF18abltfho/data_Marshal.txt',
...
spec_1 = class_spectrum.spectrum(path_to_data=
    '/home/jinlng/test_dir/Type_IIn/ZTF18abltfho/ZTF18abltfho_20180812_P200_v1.ascii',
    instrument='P200',skiprows=0,time='2018-08-12T08:02:56.157',time_format='utc',show=False)
...
spec_2 = class_spectrum.spectrum(path_to_data=
    '/home/jinlng/test_dir/Type_IIn/ZTF18abltfho/ZTF18abltfho_20180819_P60_v1.ascii',
    instrument='P60',skiprows=0,time='2018-08-19T05:52:16.377000',time_format='utc',show=False)
...
for i in range(len(spectra_cal)):
    offsets[i] = i*np.max(spectra_cal[i-1][:,1]*1.5)

phases = ['+'+str(round(spectra[i].date_jd()-explosion_date,2)) for i in range(len(spectra))]
annotations = [(spec_1_cal[-1, 0]+100,np.mean(spec_1_cal[:,1])+offsets[1]),
            (spec_2_cal[-1, 0]+100,np.min(spec_2_cal[:,1])+offsets[0])]
...
plt.title('ZTF18abltfho', fontsize=20)
...
plt.grid()
plt.tight_layout()
pylab.savefig('spectra_ZTF18abltfho.pdf', facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format='pdf',transparent=False, bbox_inches='tight')
plt.show()
