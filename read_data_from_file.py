...
def read_data_into_numpy_array(path,delimiter=None,header=False,skiprows=None,no_repeat_rows=None):
    ...
    if header==True:
        ...
        print(np.shape(pd_numpy_array))
        #print(pd_numpy_array[:, :-2])
        if no_repeat_rows==True:
            unique, index = np.unique(pd_numpy_array.astype("<U22"),return_index=True)
            print(index)
